...
function Table({ data, title, paragraph }) {
 const th = [
  ...
  { label: "Treatment", align: "right" },
  { label: "Death", align: "right" },
 ];
     ...
     <tbody>
      {data.map((item, index) => (
       <tr key={index}>
        ...
        <DataCell>
         {item.numbers?.confirmed} {item.kasus}
        </DataCell>
        <DataCell>
         {item.numbers?.recovered} {item.sembuh}
        </DataCell>
        <DataCell>
         {item.numbers?.treatment} {item.dirawat}
        </DataCell>
        <DataCell>
         {item.numbers?.death} {item.meninggal}
        </DataCell>
       </tr>
      ))}
     </tbody>
